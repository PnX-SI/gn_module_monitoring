...
def monitoring_field_to_bib_field(
    ...
    # if a field is required but hidden, we consider that it is not required since the user cannot fill it and it has to be filled with a default value
    required_value = field_data.get("required", False) and not field_data.get("hidden", True)
