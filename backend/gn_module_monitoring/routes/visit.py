...
def get_visits(object_type, module_code=None):
    ...
    if  module_code and module_code != "MONITORINGS":
        query = query.where(
            TMonitoringVisits.module.has(TMonitoringModules.module_code == module_code)
        )
    else: 
        modules_allowed = get_objet_with_permission_boolean(modules_object, object_code=OBJECT_CODE)
        ids_modules_allowed = [module["id_module"] for module in modules_allowed if module["cruved"]["R"]]
        query = query.options(joinedload(TMonitoringVisits.module)).where(
            TMonitoringVisits.id_module.in_(ids_modules_allowed)
        )
        for module in modules_allowed: 
            query = TMonitoringVisits.filter_by_readable(
                query=query, module_code=module["module_code"], object_code=OBJECT_CODE
            )
