...
from .utils import config_from_files_customized
...
def get_nomenclature(module_code):
    ...
    q = (
        ...
        .filter(BibNomenclaturesTypes.mnemonique.in_(nomenclature_types))
        .all()
    )

    return {d.id_nomenclature: d.as_dict() for d in q}
...
def get_taxonomy_from_id_list(id_list):
    ...
    q = (
        ...
        .join(
            CorNomListe, and_(CorNomListe.id_liste == id_list, CorNomListe.id_nom == Taxref.cd_nom)
        )
        .join()
        .all()
    )

    return {(d.cd_nom): (d.nom_complet) for d in q}
