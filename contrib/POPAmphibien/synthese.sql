...
--		POPAmphibien dans la synthèse
-- #############################
DROP VIEW IF EXISTS gn_monitoring.vs_POPAmphibien;
CREATE VIEW gn_monitoring.vs_POPAmphibien AS
WITH source AS (
	SELECT id_source 
	FROM gn_synthese.t_sources
	WHERE name_source = 'MONITORING_POPAMPHIBIEN' -- ici 'MONITORING_<module_path>.upper()'
	LIMIT 1
)
SELECT
	...
	WHERE m.module_path = 'POPAmphibien';
